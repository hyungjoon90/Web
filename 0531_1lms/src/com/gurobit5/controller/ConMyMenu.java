...
public class ConMyMenu extends HttpServlet {
		...
		if("list".equals(modeP)){
			...
			if("GET".equals(method_type)){
				...
				if(dtoLecInfo==null){
					...
					int[] tmp = {3,4,5 };
					if(idTypeChecker(tmp)){
						// true 일때 강의 배정할수 있는 버튼 생성 조건 올리기
	...
	private boolean idTypeChecker(int[] compareType){
		if(web_mem_type==null){	return false; }
		
		boolean result = false;
		for(int compare : compareType){
			if(compare==Integer.parseInt(web_mem_type)){
				result = true;		break;	
			}
		}// foreach end		
		return result;
