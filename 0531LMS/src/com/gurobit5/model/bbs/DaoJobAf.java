...
public class DaoJobAf {
	...
	public List<DtoJobAf> selectList(int start,int end){
		...
		String sql ="SELECT rn, name, title, lec_code, company, field, nalja, num"
				...
				+ " FROM (SELECT * FROM JOB_AF_BBS ORDER BY num DESC) WHERE rownum <= ?)"
				+ " WHERE RN >= ?";
		try {
	...
	public DtoJobAf selectOne(int num){
		DtoJobAf dtoJobAf =new DtoJobAf();
		String sql="SELECT A.NUM,A.COMPANY,B.CLASS_NAME,A.TITLE,A.NAME,A.FIELD,A.CONTENTS"
				+ " FROM JOB_AF_BBS A,LEC_INFO B"
				+ " WHERE A.LEC_CODE=B.LEC_CODE AND NUM=?";
	...
	public void insertOne(String name, String company, String class_name, String title, String field, String contents, String id){
		String sql="insert into JOB_AF_BBS"
				+ " values(JOB_AF_BBS_seq.nextval,?,(select lec_code form lec_info where class_name=?),sysdate,?,?,?,?,?,?)";
