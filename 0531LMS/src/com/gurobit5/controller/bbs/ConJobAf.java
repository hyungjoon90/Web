...
public class ConJobAf extends HttpServlet{
	...
	private void comandAdd() throws ServletException, IOException, ClassNotFoundException, SQLException{
		...
		if("GET".equals(method_type)){
			...
			String class_name=reqThis.getParameter("class_name");
			...
			DaoJobAf daoJobAf = new DaoJobAf();
			daoJobAf.insertOne(name, company, class_name, title, field, contents, contents);
			respThis.sendRedirect("?mode=list");
		}
	}// comandCreate()

	
	private void comandEdit() throws IOException, ServletException, ClassNotFoundException, SQLException {
		if("GET".equals(method_type)){
			int num=Integer.parseInt(reqThis.getParameter("idx"));
			DaoLecInfo daoLecInfo =new DaoLecInfo();
			List<DtoLecInfo> list =null;
			list=daoLecInfo.selectClassNameAll();
			reqThis.setAttribute("clist", list);
