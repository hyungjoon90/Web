...
public class ConMyMenu extends HttpServlet {
	...
	private void mymenuInit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		web_mem_type=(String) session.getAttribute("web_mem_type");
		//임시용
	...
	private void comandIdCon() throws ServletException, IOException, ClassNotFoundException, SQLException {
		...
		if("list".equals(modeP)){
			...
			if("GET".equals(method_type)){
				...
				rd = reqThis.getRequestDispatcher(goRoot+viewPath+"id_edit.jsp");
				rd.forward(reqThis, respThis);
				...
				String memTypeP = reqThis.getParameter("memType");
				String pwInitFlag = reqThis.getParameter("pwInit");

				// dto 셋팅
				int memType = Integer.parseInt(memTypeP);
				int result = 0;
				// pw에 pwinit 라는걸 만들거임
				...
				result = daoMember.changeMemTypeById(id,memType);
				if(result == 1 && "Y".equals(pwInitFlag)){
					...
					respThis.sendRedirect("?pageid=id_con&mode=list");
				}else{
					// 업데이트 실패경운데
					// 몰라 뭐할지
				}
	...
	private void comandLecCon() throws ServletException, IOException, ClassNotFoundException, SQLException, ParseException {
		...
		String[] nextUrlN = {"arrange","scorelist"}; // 05-30
		String[] nextUrlV = {"?pageid=lec_con&mode=arrange&idx=","?pageid=sc_con&mode=arrange&idx="}; // 05-30
	...
	private boolean idChecker(int[] compareType){
		if(web_mem_type==null){
			return true;
		}
