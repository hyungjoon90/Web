...
<% 
	...
	// 3. 출력 
	if(startSw)	{%>
	<a href="${wantUrl }&pageNum=1">[First]</a>
	<%}%>
	<%	if(prevSw)	{%>
	<a href="${wantUrl }&pageNum=<%=now-pageShift%>">[Prev]</a>
	<%}%>
	<% 	// 반복출력
	...
	%>
	<a
		href="${wantUrl }&pageNum=<%=addPageNum[i]%>">[<%=addPageNum[i]%>]
	</a>
	<%  		}else{
					break;
				}
	...
	<%	if(nextSw){%>
	<a href="${wantUrl }&pageNum=<%=now+pageShift%>">[Next]</a>
	<%}%>
	<%	if(endSw){%>
	<a href="${wantUrl }&pageNum=<%=pageMaxN%>">[End]</a>
	<%}%>
