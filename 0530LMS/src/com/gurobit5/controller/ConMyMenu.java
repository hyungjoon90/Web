...
public class ConMyMenu extends HttpServlet {
	...
	// 계정관리 : id_con = { try - 05-29 sskim }


	// ===== Command(modes) =========
	private String[] lecModes = {"list", "detail", "add", "edit", "del", "arrange"};
	private String[] attendModes = { "list","detail", "add", "edit", "del"};
	private String[] scoreModes = {"list","detail", "edit"};
	private String[] idModes = {"list", "typemod" ,"detail", "update"};
	...
	private String urlThis = "/pro2/mymenu.bit";
	...
	private void comandScoreCon() throws ServletException, IOException, ClassNotFoundException, SQLException {
		...
		if("list".equals(modeP)){
			...
			if("GET".equals(method_type)){
				...
				dtoScore = daoScore.selectOne(id);
			...
			if("GET".equals(method_type)){
				...
				DaoScore daoScore = new DaoScore();
				ArrayList<DtoScore> list = new ArrayList<DtoScore>();
				//list 페이지 화면 내용을 그대로 가지고 와야 한다
				list = daoScore.selectAll(id);
				
				reqThis.setAttribute("list", list);
				reqThis.setAttribute("chk", ids); 
	...
	private void comandIdCon() throws ServletException, IOException, ClassNotFoundException, SQLException {
		...
		// 계속 쓸놈들
		String[] memTypes = {"학생","강사","취업","영업","행정","관리자","예비"};
		...
		if("list".equals(modeP)){
			if("GET".equals(method_type)){
				...
				// 항목 유형 추가
				reqThis.setAttribute("memTypeListN", memTypes);
		...
		}else if("typemod".equals(modeP)){
			// 
			if("POST".equals(method_type)){
				// ajax? 이게 list로 도 다시 돌아갈순 잇음
				// q배열로 받아서 처리할거임
			...
			if("GET".equals(method_type)){
				...
				rd = reqThis.getRequestDispatcher(goRoot+viewPath+"id_detail.jsp");
				rd.forward(reqThis, respThis);
			}else{
				// 제공하지 않는 method - 0
				MyErrSender.sendErr(0, reqThis, respThis, goRoot);
			...
			if("GET".equals(method_type)){
				...
				rd = reqThis.getRequestDispatcher(goRoot+viewPath+"id_update.jsp");
				rd.forward(reqThis, respThis);
				...
				String phone = reqThis.getParameter("phone").trim();
				String email = reqThis.getParameter("email");
				String memType = reqThis.getParameter("memType");
				//String lecCode = reqThis.getParameter("lecCode"); 수정 안할거임
				String pwInitFlag = reqThis.getParameter("pwInit");

				// 유효성검사해야됨

				// dto 셋팅
				dtoMember.setId(id);
				dtoMember.setName(name);
				dtoMember.setPhone(Integer.parseInt(phone));// 주의
				dtoMember.setEmail(email);
				dtoMember.setMemType(Integer.parseInt(memType));
				int result = 0;
				// pw에 pwinit 라는걸 만들거임

				String newPw = MyRandom.getRandom(10);
				System.out.println(newPw);
				if("Y".equals(pwInitFlag)){
					if(daoMember.pwInit(id,newPw)==1){System.out.println("정상적으로 바뀜");}
				}
				result = daoMember.updateOne(dtoMember);
				if(result == 1 && "Y".equals(pwInitFlag)){
					// send 메일 하고난다음에
					// 변경된 pw 보여주기
					reqThis.setAttribute("id", id);
					reqThis.setAttribute("pw", newPw);
					rd = reqThis.getRequestDispatcher(goRoot+viewPath+"pwshow.jsp");
					rd.forward(reqThis, respThis);
				}else if(result ==1){
		...
		if("list".equals(modeP)){
			...
			if("GET".equals(method_type)){
				...
				if(dtoLecInfo==null){
					...
					reqThis.setAttribute("dtoJobInfo", dtoLecInfo);
			...
			if("GET".equals(method_type)){
				...
				daoMember.changeLecCode(dtoLecInfo.getTeacher(),dtoLecInfo.getLecCode(), 1, 0 );
			...
			if("GET".equals(method_type)){
				...
				DaoMember daoMember = new DaoMember();
				String lecCode = reqThis.getParameter("lec");
				...
				try{
					nowStulist = daoMember.selectAllStu(Integer.parseInt(lecCode));
					noneStulist = daoMember.selectAllStu(0); // 초기값
				}finally{
					// 뭐할거 생각나면 넣으셈
				}
